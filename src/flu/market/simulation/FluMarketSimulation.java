...
public class FluMarketSimulation {
    ...
    public static void main(String[] args) {
        ...
        float security_flu_rate_gaussian_dev = (float) 0.2;
        float quantity_gaussian_mean = (float) 10;
        float quantity_gaussian_dev = (float) 2.4;
            ...
            for (int j = 0; j < total_buildings; j++) {
                ...
                for (int k = 0; k < market_maker.buildings[j].participants.length; k++){
                    ...
                    for (int l = 0; l < total_buildings; l++){
                        float observed_flu_rate = get_gaussian(market_maker.buildings[l].get_flu_population_rate(), observation_gaussian_dev, 0, 1);
            ...
            for(int j=0; j<total_buildings; j++){
                for(int k=0; k<market_maker.buildings[j].participants.length; k++){
                    ...
                    for(int l=0; l<total_buildings; l++){
                        ...
                        quantity = (int) get_gaussian(quantity_gaussian_mean, quantity_gaussian_dev);
                        market_maker.buy_process(security_group_id, quantity, flu_rate, j, market_participant);                      
                    }
                }
            }
            
            //evaluate market participant's prediction accuracy in the market of date 'i'
/*
            for(int j=0; j<total_buildings; j++){
                for(int k=0; k<market_maker.buildings[j].participants.length; k++){
                    //this loop iterates through market participants of building 'j'
                    int market_participant = market_maker.buildings[j].participants[k];
                    market_maker.buildings[j].residents[market_participant].evaluate_prediction_accuracy(i);
...
*/
    ...
    public static float get_gaussian(float mean, float dev){
