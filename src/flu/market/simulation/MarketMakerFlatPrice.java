    ...
    void write_csv_population_rates(){
        ...
        try{
            for(int i=0; i<total_buildings; i++){
                ...
                //append total shares sold
                fileWriter.append(NEW_LINE_SEPERATOR);
                fileWriter.append(String.valueOf(buildings[i].total_shares_sold));
    ...
    void get_MSE(){
        for(int i=0; i<total_buildings; i++){
            ...
            for(int j=0; j<total_days; j++){
                int security_group_id = get_security_group_id(j, i);
                _MSE += (float) Math.pow(
                    ground_truths[security_group_id] - estimated_ground_truths[security_group_id], 2
                );                        
            }
